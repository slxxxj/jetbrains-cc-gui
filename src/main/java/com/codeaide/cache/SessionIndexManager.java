package com.codeaide.cache;

import com.codeaide.bridge.NodeDetector;
import com.codeaide.util.TextSanitizer;
 ...
 * Index file location: ~/.codeaide/cache/
 */
public class SessionIndexManager {
    ...
    private final Path codeaideCacheDir;
    private final Object indexFileLock = new Object();

    private static Path defaultCacheDir() {
        return Paths.get(NodeDetector.resolveHomeForFileOps(), ".codeaide", "cache");
    }
    ...
    SessionIndexManager(Path codeaideCacheDir) {
        this.codeaideCacheDir = codeaideCacheDir;
        ensureCacheDir();
    }
    ...
    private void ensureCacheDir() {
        try {
            if (!Files.exists(codeaideCacheDir)) {
                Files.createDirectories(codeaideCacheDir);
                LOG.info("[SessionIndexManager] Created cache directory: " + codeaideCacheDir);
            }
        } catch (IOException e) {
    ...
    public Path getClaudeIndexPath() {
        return codeaideCacheDir.resolve(CLAUDE_INDEX_FILE);
    }
    ...
    public Path getCodexIndexPath() {
        return codeaideCacheDir.resolve(CODEX_INDEX_FILE);
    }
