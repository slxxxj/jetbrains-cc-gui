...
import com.codeaide.settings.ProviderManager;
...
public class ClaudeProviderOperations {
    ...
    public void handleAddProvider(String content) {
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                context.callJavaScript("window.showError", context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("provider.addFailed", e.getMessage())));
            });
        }
    }
    ...
    public void handleUpdateProvider(String content) {
        try {
            ...
            // Phase 5c: no settings.json re-sync needed — managed provider
            // credentials live in ~/.codeaide/config.json and the bridge reads
            // them from there on every request.
            ApplicationManager.getApplication().invokeLater(() -> {
                handleGetProviders(); // Refresh list
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                context.callJavaScript("window.showError", context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("provider.updateFailed", e.getMessage())));
            });
        }
    }
    ...
    public void handleDeleteProvider(String content) {
        ...
        try {
            ...
            if (!data.has("id")) {
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    context.callJavaScript("window.showError", context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("provider.deleteMissingId")));
                });
                return;
            }
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                context.callJavaScript("window.showError", context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("provider.deleteFailed", e.getMessage())));
            });
        }
    ...
    public void handleSwitchProvider(String content) {
        try {
            ...
            // ~/.codeaide/config.json) and the Agent SDK spawns its CLI subprocess at
            ...
            if (ProviderManager.DISABLED_PROVIDER_ID.equals(id)) {
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    context.callJavaScript("window.showSwitchSuccess",
                            context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("toast.providerDisabled")));
                    handleGetProviders();
            ...
            if (ProviderManager.LOCAL_SETTINGS_PROVIDER_ID.equals(id)) {
                ...
                if (!Files.exists(settingsPath)) {
                    ...
                    ApplicationManager.getApplication().invokeLater(() -> {
                        context.callJavaScript("window.showError",
                                context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("error.localProviderSettingsNotFound")));
                    });
                    return;
                }
                ...
                try {
                    ...
                    ApplicationManager.getApplication().invokeLater(() -> {
                        context.callJavaScript("window.showError",
                                context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("error.localProviderInvalidJson", e.getMessage())));
                    });
                    return;
                }
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    context.callJavaScript("window.showSwitchSuccess",
                            context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("toast.localProviderSwitchSuccess")));
                    handleGetProviders();
                    handleGetActiveProvider();
                });
                return;
            ...
            if (ProviderManager.CLI_LOGIN_PROVIDER_ID.equals(id)) {
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    context.callJavaScript("window.showSwitchSuccess",
                            context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("toast.cliLoginSwitchSuccess")));
                    handleGetProviders();
                    handleGetActiveProvider();
            ...
            // Phase 5c: credentials stay in ~/.codeaide/config.json; nothing is
            // written to ~/.claude/settings.json on provider switch.

            ApplicationManager.getApplication().invokeLater(() -> {
                String successMsg = com.codeaide.i18n.CodeAideBundle.message("toast.providerSwitchSuccess")
                        + com.codeaide.i18n.CodeAideBundle.message("provider.switchSyncClaude");
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                context.callJavaScript("window.showError", context.escapeJs(com.codeaide.i18n.CodeAideBundle.message("toast.providerSwitchFailed") + ": " + e.getMessage()));
            });
        }
    }
