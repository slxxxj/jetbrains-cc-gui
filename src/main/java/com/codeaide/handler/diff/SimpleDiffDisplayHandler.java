...
import com.codeaide.util.ContentRebuildUtil;
import com.codeaide.util.EditorFileUtils;
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    String diffTitle = title != null ? title : CodeAideBundle.message("diff.fileChange", fileName);
                    SimpleDiffRequest diffRequest = new SimpleDiffRequest(
                            diffTitle,
                            leftContent,
                            rightContent,
                            CodeAideBundle.message("diff.editBefore", fileName),
                            CodeAideBundle.message("diff.editAfter", fileName)
                    );
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    String diffTitle = CodeAideBundle.message("diff.tabName", fileName, edits.size());
                    SimpleDiffRequest diffRequest = new SimpleDiffRequest(
                            diffTitle,
                            leftContent,
                            rightContent,
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    String diffTitle = title != null ? title : CodeAideBundle.message("diff.editPreview", fileName);
                    SimpleDiffRequest diffRequest = new SimpleDiffRequest(
                            diffTitle,
                            leftContent,
                            rightContent,
                            CodeAideBundle.message("diff.editPreviewCurrent", fileName),
                            CodeAideBundle.message("diff.editPreviewAfter", fileName)
                    );
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                try {
                    ...
                    if (originalContent == null || originalContent.isEmpty()) {
                        diffTitle = (title != null ? title : CodeAideBundle.message("diff.edit", fileName))
                                + " " + CodeAideBundle.message("diff.editOnly");
                        leftLabel = "old_string";
                        rightLabel = "new_string";
                    } else {
                        diffTitle = title != null ? title : CodeAideBundle.message("diff.edit", fileName);
                        leftLabel = CodeAideBundle.message("diff.editBefore", fileName);
                        rightLabel = CodeAideBundle.message("diff.editAfter", fileName);
                    }
