...
import com.codeaide.settings.CodexMcpServerManager;
 ...
 * Configuration is stored in the effective Codex home's config.toml
 * (~/.codeaide/codex-home for managed providers, ~/.codex in CLI login mode)
 */
public class CodexMcpServerHandler extends BaseMessageHandler {
    ...
    private void handleGetMcpServerTools(String content) {
        try {
            if (!isCodexLocalConfigAuthorized()) {
                Gson gson = new Gson();
                sendToolsError("", com.codeaide.i18n.CodeAideBundle.message("error.codexLocalAccessNotAuthorized"), gson);
                return;
            }
