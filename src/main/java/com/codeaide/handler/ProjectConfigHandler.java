...
import com.codeaide.action.SendShortcutSync;
import com.codeaide.provider.claude.ClaudeHistoryReader;
import com.codeaide.provider.codex.CodexHistoryReader;
...
public class ProjectConfigHandler {
    ...
    public void handleGetPermissionDialogTimeout() {
        respondWithJson("window.updatePermissionDialogTimeout",
            ...
            jsonOf("permissionDialogTimeoutSeconds", CodeaideSettingsService.DEFAULT_PERMISSION_DIALOG_TIMEOUT_SECONDS),
    ...
    JsonObject setPermissionDialogTimeoutAndCreateResponse(String content) throws Exception {
        ...
        int seconds = CodeaideSettingsService.DEFAULT_PERMISSION_DIALOG_TIMEOUT_SECONDS;
    ...
    public void handleGetPromptEnhancerConfig() {
        try {
            ...
            showError(CodeAideBundle.message("projectConfig.promptEnhancer.getFailed", e.getMessage()));
        }
    }
    ...
    public void handleGetCommitAiConfig() {
        try {
            ...
            showError(CodeAideBundle.message("projectConfig.commitAi.getFailed", e.getMessage()));
        }
    }
    ...
    private void applyAiProviderConfig(String content, AiProviderSetter setter,
        ...
        try {
            ...
            showError(CodeAideBundle.message(errorBundleKey, e.getMessage()));
        }
    }
    ...
    public void handleGetCommitAgentConfig() {
        respondWithJson("window.updateCommitAgentConfig",
            () -> {
                JsonObject json = new JsonObject();
                json.addProperty("batchSize", settingsService.getCommitAgentBatchSize());
                json.addProperty("maxParallel", settingsService.getCommitAgentMaxParallel());
                json.addProperty("fastMode", settingsService.getCommitFastMode());
                json.addProperty("includeFileDetail", settingsService.getCommitIncludeFileDetail());
                return json;
            },
            defaultCommitAgentConfig(),
            "Failed to get commit agent config");
    }

    public void handleSetCommitAgentConfig(String content) {
        try {
            JsonObject json = gson.fromJson(content, JsonObject.class);
            if (json != null) {
                if (json.has("batchSize") && !json.get("batchSize").isJsonNull()) {
                    settingsService.setCommitAgentBatchSize(json.get("batchSize").getAsInt());
                }
                if (json.has("maxParallel") && !json.get("maxParallel").isJsonNull()) {
                    settingsService.setCommitAgentMaxParallel(json.get("maxParallel").getAsInt());
                }
                if (json.has("fastMode") && !json.get("fastMode").isJsonNull()) {
                    settingsService.setCommitFastMode(json.get("fastMode").getAsBoolean());
                }
                if (json.has("includeFileDetail") && !json.get("includeFileDetail").isJsonNull()) {
                    settingsService.setCommitIncludeFileDetail(json.get("includeFileDetail").getAsBoolean());
                }
            }
            LOG.info("[ProjectConfigHandler] Set commit agent config: " + content);
            // Echo the effective (clamped) values back so the UI stays in sync
            handleGetCommitAgentConfig();
        } catch (Exception e) {
            LOG.error("[ProjectConfigHandler] Failed to set commit agent config: " + e.getMessage(), e);
            showError("Failed to save commit agent config");
        }
    }

    private static JsonObject defaultCommitAgentConfig() {
        JsonObject json = new JsonObject();
        json.addProperty("batchSize", CodeaideSettingsService.DEFAULT_COMMIT_AGENT_BATCH_SIZE);
        json.addProperty("maxParallel", CodeaideSettingsService.DEFAULT_COMMIT_AGENT_MAX_PARALLEL);
        json.addProperty("fastMode", CodeaideSettingsService.DEFAULT_COMMIT_FAST_MODE);
        json.addProperty("includeFileDetail", CodeaideSettingsService.DEFAULT_COMMIT_INCLUDE_FILE_DETAIL);
        return json;
    }
