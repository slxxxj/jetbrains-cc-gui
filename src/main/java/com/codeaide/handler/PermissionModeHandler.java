    ...
    public void handleSetMode(String content) {
        try {
            ...
            if (this.context.getSession() != null) {
                ...
                com.codeaide.notifications.ClaudeNotifier.setMode(this.context.getProject(), mode);
    ...
    private void pushPermissionModeLive(String mode) {
        try {
            ...
            if (provider == null || provider.isEmpty()) {
                provider = com.codeaide.handler.core.HandlerContext.DEFAULT_PROVIDER;
            }

            // Only runtimes with live mode-swap support (today: the Claude
            // persistent runtime) can hot-swap the mode on a live query; Codex
            // rebuilds thread options per turn.
            ...
            com.codeaide.provider.common.ProviderOps ops = this.context.getProviderOps(provider);
            if (!ops.capabilities().supportsLivePermissionMode()) {
                return;
            }

            com.codeaide.session.ClaudeSession session = this.context.getSession();
            if (session == null) {
                return;
            }
            ...
            ops.setPermissionModeLive(sessionId, epoch, mode)
                    .exceptionally(ex -> {
