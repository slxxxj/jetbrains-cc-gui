...
public class LanguageConfigService {
    ...
    /**
     * Map an i18n language code to the Locale used for Java-side resource bundles.
     * Codes without a dedicated bundle (ko, pt-BR) resolve to the base bundle (English).
     *
     * @param languageCode i18n language code (zh, en, zh-TW, ...)
    ...
    /**
     * Get a human-readable display name for an i18n language code, suitable for
     * embedding in AI prompts. English name plus native name to avoid ambiguity.
     *
     * @param languageCode i18n language code (zh, en, zh-TW, ...)
     * @return display name such as "Simplified Chinese (简体中文)"
     */
     ...
     */
    public static String getUserLanguage(CodeaideSettingsService settingsService) {
        ...
        try {
            ...
            if (!SUPPORTED_LANGUAGES.contains(userLanguage)) {
                LOG.warn("[LanguageConfig] Ignoring unsupported user language in ~/.codeaide/config.json: " + userLanguage);
                return null;
            }
            LOG.info("[LanguageConfig] User manually set language: " + userLanguage);
            return userLanguage;
        } catch (Exception e) {
            LOG.warn("[LanguageConfig] Failed to read user language from ~/.codeaide/config.json: " + e.getMessage());
            return null;
        }
    }
     ...
     */
    public static void setUserLanguage(CodeaideSettingsService settingsService, String language) throws IOException {
        if (settingsService == null) {
     ...
     */
    public static void clearUserLanguage(CodeaideSettingsService settingsService) throws IOException {
        if (settingsService == null) {
            throw new IllegalArgumentException("settingsService must not be null");
        }
     ...
     */
    public static JsonObject getLanguageConfig(CodeaideSettingsService settingsService) {
     ...
     */
    public static String getLanguageConfigJson(CodeaideSettingsService settingsService) {
     ...
     */
    public static String getCurrentLanguage(CodeaideSettingsService settingsService) {
        String userLanguage = getUserLanguage(settingsService);
        if (userLanguage != null && !userLanguage.isEmpty()) {
            return userLanguage;
        }
