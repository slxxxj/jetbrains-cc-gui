...
import com.codeaide.util.HtmlLoader;
import com.codeaide.util.JsUtils;
import com.codeaide.util.JBCefBrowserFactory;
...
public class WebviewInitializer {
    ...
    public void createUIComponents() {
        ...
        com.codeaide.bridge.BridgeDirectoryResolver sharedResolver = BridgePreloader.getSharedResolver();
        ...
        com.codeaide.model.NodeDetectionResult nodeResult = null;
        ...
        try {
            ...
            getClipboardPathQuery.addHandler((msg) -> {
                // Async pattern (mirrors ClipboardHandler.handleReadClipboard, the
                // issue #846 fix): the CEF callback thread must never touch the
                // system clipboard — on Windows the OLE clipboard lock can block it
                // for seconds and freeze the whole WebView.  Return immediately and
                // read the clipboard on the EDT instead; the result is pushed back
                // to JS via window.__onClipboardFilePath.  ModalityState.any() keeps
                // paste working while a modal dialog is open.
                final String requestId = msg;
                ApplicationManager.getApplication().invokeLater(() -> {
                    String filePath = readClipboardFilePath();
                    try {
                        CefBrowser cefBrowser = browser.getCefBrowser();
                        cefBrowser.executeJavaScript(
                                JsUtils.buildJsCall("__onClipboardFilePath",
                                        JsUtils.escapeJs(requestId), JsUtils.escapeJs(filePath)),
                                cefBrowser.getURL(), 0);
                    } catch (Exception ex) {
                        LOG.warn("Failed to push clipboard path to webview: " + ex.getMessage());
                    }
                }, ModalityState.any());
                return new JBCefJSQuery.Response("ok");
            });

            // Create a dedicated JSQuery for hiding the CodeAide panel via Shift+Esc
            ...
            hidePanelQuery.addHandler((msg) -> {
                try {
                    ...
                    if (project != null && !project.isDisposed()) {
                        ApplicationManager.getApplication().invokeLater(() -> {
                            ToolWindow toolWindow = ToolWindowManager.getInstance(project).getToolWindow("CodeAide");
                    ...
                    LOG.warn("Failed to hide CodeAide panel via shortcut: " + ex.getMessage());
                }
                return new JBCefJSQuery.Response("ok");
            });
            ...
            String htmlContent = loadHtmlWithTabState();

            // Report render-process death (crash / kill / OOM) to the watchdog
            // immediately instead of waiting for the 45s heartbeat timeout.
            // CefRequestHandlerAdapter's other methods use pass-through defaults,
            // so browsing and resource handling are unaffected.
            browser.getJBCefClient().addRequestHandler(new CefRequestHandlerAdapter() {
                ...
                public void onRenderProcessTerminated(CefBrowser cefBrowser,
                    ...
                    host.getWebviewWatchdog().notifyRenderProcessTerminated(String.valueOf(status));
                }
            }, browser.getCefBrowser());
            ...
            browser.getJBCefClient().addLoadHandler(new CefLoadHandlerAdapter() {
                ...
                public void onLoadEnd(CefBrowser cefBrowser, CefFrame frame, int httpStatusCode) {
                    ...
                    // Arm the watchdog's "alive but blank" detection: from now on,
                    // frontend_ready must arrive within the ready-timeout window.
                    host.getWebviewWatchdog().markLoadCompleted();
                    ...
                    // Inject clipboard path retrieval function.  The Java side
                    // answers asynchronously (see the getClipboardPathQuery handler),
                    // so each request carries a sequence id and the result arrives
                    // via window.__onClipboardFilePath, which resolves the matching
                    // pending promise.  The public contract is unchanged:
                    // window.getClipboardFilePath() returns Promise<string>.
                    String clipboardPathInjection =
                        "window.__clipboardFilePathRequests = {};" +
                        "window.__clipboardFilePathSeq = 0;" +
                        "window.__onClipboardFilePath = function(id, path) {" +
                        "  var resolve = window.__clipboardFilePathRequests[id];" +
                        "  if (resolve) {" +
                        "    delete window.__clipboardFilePathRequests[id];" +
                        "    resolve(path || '');" +
                        "  }" +
                        "};" +
                        "window.getClipboardFilePath = function() {" +
                        "  return new Promise((resolve) => {" +
                        "    var id = String(++window.__clipboardFilePathSeq);" +
                        "    window.__clipboardFilePathRequests[id] = resolve;" +
                        "    " + getClipboardPathQuery.inject("id",
                            "function(response) {}",
                            "function(error_code, error_message) { console.error('Failed to get clipboard path:', error_message); window.__onClipboardFilePath(id, ''); }") +
                        "  });" +
                        "};";
                    ...
                    if (PlatformUtils.isPluginDevMode()) {
                        String consoleForward =
                            ...
                            "  window.sendToJava(JSON.stringify({type: 'console_log', payload: {level: 'log', args: args}}));" +
                            "};" +
                            "console.error = function(...args) {" +
                            "  originalError.apply(console, args);" +
                            "  window.sendToJava(JSON.stringify({type: 'console_log', payload: {level: 'error', args: args}}));" +
                            "};" +
                            "console.warn = function(...args) {" +
                            "  originalWarn.apply(console, args);" +
                            "  window.sendToJava(JSON.stringify({type: 'console_log', payload: {level: 'warn', args: args}}));" +
                            "};";
                ...
                public void onLoadError(CefBrowser cefBrowser, CefFrame frame,
                    ...
                    // The watchdog filters: only main-frame, non-ERR_ABORTED errors
                    // (ERR_ABORTED fires normally when a reload interrupts a load).
                    host.getWebviewWatchdog().notifyLoadError(
    ...
    /**
     * Read the first file path from the system clipboard (blocking AWT access —
     * must be called off the CEF callback thread; see the getClipboardPathQuery
     * handler).  Returns "" when the clipboard holds no files or read fails.
     * Package-private for tests.
     */
    static String readClipboardFilePath() {
        try {
    ...
    private void showVersionErrorPanel(String currentVersion) {
        ...
        JPanel errorPanel = ErrorPanelBuilder.build(
            CodeAideBundle.message("error.nodeVersionTooOld.title"),
            message,
            claudeSDKBridge.getNodeExecutable(),
            this::handleNodePathSave
        );
    ...
    private void showJcefNotSupportedPanel() {
        ...
        if (JBCefBrowserFactory.isJbrMissingJcefRemoteApi()) {
            ...
            title = CodeAideBundle.message("toolwindow.jcefOutdatedJbr");
            solution = CodeAideBundle.message("toolwindow.jcefOutdatedJbrSolution");
        } else {
            title = CodeAideBundle.message("toolwindow.jcefNotInstalled");
            solution = CodeAideBundle.message("toolwindow.jcefNotInstalledSolution");
        }
    ...
    private void showJcefRemoteModeErrorPanel() {
        JPanel panel = ErrorPanelBuilder.buildCenteredPanel(
            "⚠️",
            CodeAideBundle.message("toolwindow.jcefRemoteError"),
            CodeAideBundle.message("toolwindow.jcefRemoteSolution")
        );
        replaceMainContent(panel);
    }
    ...
    private void showLoadingPanel() {
        JPanel panel = ErrorPanelBuilder.buildLoadingPanel(
            "⏳",
            CodeAideBundle.message("toolwindow.extractingTitle"),
            CodeAideBundle.message("toolwindow.extractingDesc")
        );
        replaceMainContent(panel);
    ...
    /**
     * Load the chat HTML with the per-tab provider/model injected (issue #1353).
     * Shared by the initial load and the watchdog's reload path so a recovery
     * cannot silently revert the tab to the global localStorage provider.
     */
    private String loadHtmlWithTabState() {
    ...
    public void reloadWebview(String reason) {
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            try {
                browser.loadHTML(loadHtmlWithTabState());
                host.getWebviewWatchdog().resetTimestamps();
