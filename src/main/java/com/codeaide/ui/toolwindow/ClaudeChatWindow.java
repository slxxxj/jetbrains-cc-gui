package com.codeaide.ui.toolwindow;
...
import com.codeaide.session.SessionCallbackAdapter;
...
import com.codeaide.settings.TabStateService;
import com.codeaide.ui.ChatWindowDelegate;
import com.codeaide.ui.EditorContextTracker;
import com.codeaide.ui.WebviewInitializer;
import com.codeaide.ui.WebviewWatchdog;
...
public class ClaudeChatWindow {
    ...
    public ClaudeChatWindow(Project project, boolean skipRegister) {
        ...
        this.mainPanel.setBackground(com.codeaide.util.ThemeConfigService.getBackgroundColor());
        ...
        this.webviewWatchdog = new WebviewWatchdog(
                mainPanel,
                reason -> webviewInitializer.reloadWebview(reason),
    ...
    /**
     * Watchdog hook for tab selection: a crash or "alive but blank" state on a
     * background tab is not recovered proactively (its panel is not showing),
     * so it is picked up here the moment the user switches to the tab.
     */
    public void checkWebviewHealthOnTabSelected() {
        webviewWatchdog.checkHealthOnTabSelected();
    }
    ...
    void callJavaScript(String functionName, String... args) {
        ...
        final String js;
        try {
            js = JsUtils.buildJsCall(functionName, args);
    ...
    void handleJavaScriptMessage(String message) {
        ...
        String type;
        try {
            json = new Gson().fromJson(message, JsonObject.class);
            type = json.get("type").getAsString();
        } catch (Exception e) {
            LOG.warn("Failed to parse bridge message: " + e.getMessage());
            return;
        }

        JsonElement payload = json.get("payload");

        // Forwarded webview console logs (dev mode only) arrive as
        // {type: 'console_log', payload: {level, args}} and bypass the dispatcher.
        if ("console_log".equals(type)) {
            handleConsoleLog(payload);
            return;
        }

        // Handlers keep their historical String input: object/array payloads are
        // delivered as JSON text, primitives as their raw text, missing as "".
        String content;
        if (payload == null || payload.isJsonNull()) {
            content = "";
        } else if (payload.isJsonPrimitive()) {
            content = payload.getAsString();
        } else {
    ...
    private void onStreamEnded() {
        ...
        if ("claude".equals(session.getProvider()) && session.getError() == null) {
            com.codeaide.notifications.ClaudeNotifier.showSuccess(
                project,
                com.codeaide.notifications.ClaudeNotifier.buildTitleFromSession(session),
                com.codeaide.notifications.ClaudeNotifier.buildPreviewFromSession(session, "Task completed"));
        }
    }
    ...
    private void interruptDueToPermissionDenial() {
        this.session.interrupt().thenRun(() -> ApplicationManager.getApplication().invokeLater(() -> {
            ...
            com.codeaide.notifications.ClaudeNotifier.clearStatus(project);
        }));
    }
