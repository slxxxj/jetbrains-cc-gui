...
public class NodeDetectionResult {
    ...
    public enum DetectionMethod {
        /** Plugin-managed runtime auto-downloaded to ~/.codeaide/runtime/node */
        PLUGIN_MANAGED,
    ...
    public String getUserFriendlyMessage() {
        ...
        // The plugin can auto-provision an isolated runtime via NodeRuntimeManager, so the
        // primary guidance is "wait for the automatic download" rather than "go install
        // Node.js yourself". Manual installation is kept as a fallback for offline /
        // restricted-network environments where the automatic download cannot work.
        java.nio.file.Path managedExe = NodeRuntimeManager.getInstance().getNodeExecutablePath();
        if (managedExe != null) {
            sb.append("插件正在后台自动下载一个隔离的 Node.js 运行时（约 30MB，无需手动安装、不影响系统环境）：\n");
            sb.append("  ").append(managedExe).append("\n");
            sb.append("下载完成后会自动生效。若网络受限导致自动下载失败，再参考下面的手动安装建议。\n\n");
        } else {
            sb.append("当前平台不支持自动下载 Node.js 运行时，请参考下面的手动安装建议。\n\n");
        }
