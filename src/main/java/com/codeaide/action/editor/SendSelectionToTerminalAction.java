...
public class SendSelectionToTerminalAction extends AnAction implements DumbAware {
    ...
    SendSelectionToTerminalAction(@NotNull SelectionReferenceBuilder selectionReferenceBuilder) {
        super(
            CodeAideBundle.message("action.sendToGui.text"),
            CodeAideBundle.message("action.sendToGui.description"),
            null
        );
     ...
     * Without selection: just opens the CodeAide panel.
     */
    @Override
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        try {
            ...
            showError(project, CodeAideBundle.message("send.failed", ex.getMessage()));
    ...
    private void sendToChatWindow(@NotNull Project project, @NotNull String text) {
        try {
            ...
            ToolWindow toolWindow = toolWindowManager.getToolWindow("CodeAide");
            ...
            if (toolWindow != null) {
                ...
                showError(project, CodeAideBundle.message("send.toolWindowNotFound"));
            }

        } catch (Exception ex) {
            showError(project, CodeAideBundle.message("send.sendToChatFailed", ex.getMessage()));
    ...
    private void showError(@Nullable Project project, @NotNull String message) {
        ...
        if (project != null) {
            ApplicationManager.getApplication().invokeLater(() -> {
                com.intellij.openapi.ui.Messages.showErrorDialog(project, message, CodeAideBundle.message("dialog.error.title"));
            });
        }
    }
    ...
    private void showInfo(@Nullable Project project, @NotNull String message) {
        ...
        if (project != null) {
            ApplicationManager.getApplication().invokeLater(() -> {
                com.intellij.openapi.ui.Messages.showInfoMessage(project, message, CodeAideBundle.message("dialog.info.title"));
            });
        }
    }

    /**
     * Activate the CodeAide tool window and focus the input field.
     */
