package com.codeaide.action.dev;
...
public class OpenDevToolsAction extends AnAction {
    ...
    public OpenDevToolsAction() {
        super(
            CodeAideBundle.message("action.openDevTools.text"),
            CodeAideBundle.message("action.openDevTools.description"),
            null
        );
    }
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        List<DevToolsOption> options = List.of(
                new DevToolsOption(CodeAideBundle.message("devtools.openEmbedded"), this::openEmbeddedDevTools),
                new DevToolsOption(CodeAideBundle.message("devtools.openChrome"), this::openChromeDevTools),
                new DevToolsOption(CodeAideBundle.message("devtools.copyDebugInfo"), this::copyDebugInfo)
        );

        ListPopup popup = JBPopupFactory.getInstance().createListPopup(
                new BaseListPopupStep<>(CodeAideBundle.message("devtools.popupTitle"), options) {
                    @Override
    ...
    private void openEmbeddedDevTools(Project project) {
        ...
        if (browser == null) {
            LOG.warn("[OpenDevToolsAction] No JBCefBrowser found in CodeAide tool window");
            showNotification(project, CodeAideBundle.message("devtools.noBrowser"), NotificationType.WARNING);
            return;
        }
            ...
            if (browser.getCefBrowser() != null) {
                ApplicationManager.getApplication().invokeLater(() -> {
                    try {
                        ...
                        showNotification(project, CodeAideBundle.message("devtools.openFailed", ex.getMessage()), NotificationType.ERROR);
                    }
                });
            }
        } catch (Exception ex) {
    ...
    private void openChromeDevTools(Project project) {
        ...
        if (port <= 0) {
            showNotification(project,
                    CodeAideBundle.message("devtools.portNotAvailable"),
        ...
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
            try {
                ...
                if (jsonResponse == null || jsonResponse.isEmpty()) {
                    ApplicationManager.getApplication().invokeLater(() ->
                            showNotification(project, CodeAideBundle.message("devtools.noResponse", port), NotificationType.ERROR));
                    return;
                }
                ...
                if (devtoolsUrl != null) {
                    ...
                    ApplicationManager.getApplication().invokeLater(() -> {
                        ...
                        showNotification(project,
                                CodeAideBundle.message("devtools.urlNotFound"),
                ...
                ApplicationManager.getApplication().invokeLater(() ->
                        showNotification(project, CodeAideBundle.message("devtools.fetchFailed", ex.getMessage()), NotificationType.ERROR));
            }
        });
    }
    ...
    private void copyDebugInfo(Project project) {
        ...
        String debugInfo = CodeAideBundle.message("devtools.debugInfoTitle") + "\n" +
                "━━━━━━━━━━━━━━━━━━━━━━\n" +
                "Port: " + port + "\n" +
                CodeAideBundle.message("devtools.debugInfoTargetList", port) + "\n" +
                "━━━━━━━━━━━━━━━━━━━━━━\n\n" +
                CodeAideBundle.message("devtools.debugInfoInstructions", port);

        copyToClipboard(debugInfo);
        showNotification(project, CodeAideBundle.message("devtools.debugInfoCopied", port), NotificationType.INFORMATION);
    ...
    private void showNotification(Project project, String message, NotificationType type) {
        NotificationGroupManager.getInstance()
                .getNotificationGroup("CodeAide Notifications")
