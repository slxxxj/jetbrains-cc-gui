...
import com.codeaide.ui.detached.DetachedChatFrame;
...
/**
 * Action to detach a tab from the CodeAide tool window and show it in a floating window.
...
public class DetachTabAction extends AnAction implements DumbAware {
    ...
    public DetachTabAction() {
        super(
                CodeAideBundle.message("action.detachTab.text"),
                CodeAideBundle.message("action.detachTab.description"),
                null
        );
    }
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (chatWindow == null) {
            ...
            Messages.showErrorDialog(
                    project,
                    CodeAideBundle.message("action.detachTab.error.noChatWindow"),
                    CodeAideBundle.message("action.detachTab.error.title")
            );
            return;
        }
        ...
        if (sessionId != null && DetachedWindowManager.isDetached(project, sessionId)) {
            ...
            Messages.showInfoMessage(
                    project,
                    CodeAideBundle.message("action.detachTab.error.alreadyDetached"),
                    CodeAideBundle.message("action.detachTab.error.title")
            );
            return;
        }
        ...
        if (contentManager.getContentCount() <= 1) {
            ...
            Messages.showWarningDialog(
                    project,
                    CodeAideBundle.message("action.detachTab.error.lastTab"),
                    CodeAideBundle.message("action.detachTab.error.title")
            );
            return;
        }
    ...
    private void detachTab(@NotNull Project project,
        ...
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            try {
                ...
                Messages.showErrorDialog(
                        project,
                        CodeAideBundle.message("action.detachTab.error.failed") + ": " + ex.getMessage(),
                        CodeAideBundle.message("action.detachTab.error.title")
                );
            } finally {
