...
public class SkillManager {
     ...
     *
     * Phase 5c: skipped while a managed provider is active — then the plugin no
     * longer writes ~/.claude/settings.json, and the ai-bridge passes enabled
     * skills from ~/.codeaide/config.json to the SDK via its `plugins` option
     * instead. Local settings / CLI login modes keep the legacy settings.json
     * sync untouched.
     */
    public void syncSkillsToClaudeSettings() throws IOException {
        if (isManagedClaudeProviderActive()) {
            LOG.info("[SkillManager] Managed provider active, skipping skills sync to ~/.claude/settings.json");
            return;
        }
     ...
     * decided solely by the codeaide config (claude.current points at a real
     * provider entry rather than a virtual provider id).
     */
    private boolean isManagedClaudeProviderActive() {
        try {
            ...
            return claude.has("providers") && claude.get("providers").isJsonObject()
                    && claude.getAsJsonObject("providers").has(currentId);
        } catch (Exception e) {
            LOG.debug("[SkillManager] Failed to resolve Claude provider mode: " + e.getMessage());
            return false;
        }
    }
}
