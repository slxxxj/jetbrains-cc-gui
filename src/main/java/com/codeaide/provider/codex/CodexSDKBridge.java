...
import com.codeaide.handler.history.HistoryMessageInjector;
...
import com.codeaide.bridge.ProcessManager;
...
public class CodexSDKBridge extends BaseSDKBridge {
    ...
    private final CodeaideSettingsService settingsService = new CodeaideSettingsService();
    ...
    protected void processEnvelope(
        ...
        switch (type) {
            ...
            case "session_id":
                // Codex thread ids arrive under the unified 'session_id' type.
                callback.onMessage("session_id", envelopePayloadAsString(data));
                return;
            ...
            case "content_delta": {
                String delta = envelopePayloadAsString(data);
                assistantContent.append(delta);
                callback.onMessage("content_delta", delta);
                return;
            }
            ...
            case "content": {
                String content = envelopePayloadAsString(data);
                // Avoid duplicate
                if (!assistantContent.toString().contains(content)) {
                    assistantContent.append(content);
                }
            ...
            case "send_error": {
                String errorMessage = envelopePayloadAsString(data);
                if (data != null && data.isJsonObject()) {
            ...
            case "node_error":
                // Mirrors the legacy [STDIN_ERROR]-style tag lines; keeps the
                // "[SOURCE] message" text shape for error reporting.
                if (data != null && data.isJsonObject()) {
                    ...
                    lastNodeError.set("[" + source + "] " + message);
                } else {
                    lastNodeError.set("[NODE_ERROR] " + envelopePayloadAsString(data));
                }
                return;

            default:
                // Unknown envelope types are ignored (forward-compatible).
        }
    }

    private String envelopePayloadAsString(JsonElement data) {
     ...
     * Codex SDK is installed in ~/.codeaide/dependencies/codex-sdk/, not in ai-bridge.
     */
    private void setCodexExecutablePermission(File bridgeDir) {
        try {
            // Codex SDK is installed in ~/.codeaide/dependencies/codex-sdk/
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                if (CodeaideSettingsService.CODEX_RUNTIME_ACCESS_INACTIVE.equals(accessMode)) {
                    String error = CodeAideBundle.message("error.codexLocalAccessNotAuthorized");
                    result.success = false;
                    result.error = error;
                ...
                try {
                    ...
                    try (BufferedReader reader = new BufferedReader(
                        ...
                        while ((line = reader.readLine()) != null) {
                            ...
                            // Structured v2 envelopes ({type, data}) take
                            // precedence over legacy tag lines.
                            if (tryProcessEnvelope(line, callback, result, assistantContent, hadSendError, lastNodeError)) {
                                continue;
                            }
