...
import com.codeaide.handler.ClaudeCliPathHandler;
 ...
 * - Out-of-band permission prompts arrive as {"type":"permission_request",...}
 *   (no request id) and are answered via {@link #sendOutOfBand} with
 *   {"type":"permission_response",...}; both bypass the command queue
...
public class DaemonBridge {
    ...
    // Additional lifecycle listeners. CopyOnWriteArrayList allows safe
    // iteration during dispatch while listeners may be added/removed
    // concurrently (e.g. PermissionService toggling its file watcher on
    // daemon ready/died).
    private final List<DaemonLifecycleListener> lifecycleListeners = new CopyOnWriteArrayList<>();
    ...
    public boolean start() {
        synchronized (startLock) {
            ...
            try {
                ...
                // Configure environment. Daemon mode does NOT inject the
                // permission file-IPC dir: permission prompts ride the
                // out-of-band NDJSON channel handled by this class.
                Map<String, String> env = pb.environment();
                envConfigurator.updateDaemonProcessEnvironment(pb, nodePath);
    ...
    /**
     * Send an out-of-band message to the daemon's stdin. The message bypasses
     * the daemon's command queue (same side channel as abort) and carries no
     * request id, so it is never queued behind an in-flight turn. Used for
     * permission_response decisions answering a permission_request prompt.
     */
    public void sendOutOfBand(JsonObject message) {
        try {
            if (daemonStdin != null && isRunning.get()) {
                synchronized (daemonStdin) {
                    daemonStdin.write(message.toString());
                    daemonStdin.newLine();
                    daemonStdin.flush();
                }
                LOG.debug("[DaemonBridge] Sent out-of-band message: " + message.get("type"));
            }
        } catch (IOException e) {
            LOG.debug("[DaemonBridge] Error sending out-of-band message: " + e.getMessage());
        }
    }
    ...
    void handleDaemonOutput(String jsonLine) {
        markDaemonActivity();
        ...
        try {
            ...
            if (obj.has("type")) {
                ...
                if ("permission_request".equals(type)) {
                    // Out-of-band permission prompt (not tagged with a request
                    // id — a canUseTool prompt can surface mid-turn or between
                    // turns). Routed to event listeners (PermissionService);
                    // the decision goes back via sendOutOfBand.
                    notifyEventListeners("permission_request", obj);
                    return;
                }
            ...
            // Structured v2 envelope: {"id":"X","type":"...","data":...}
            // (daemon/heartbeat/status types were already handled above)
            if (obj.has("type") && obj.get("type").isJsonPrimitive()) {
                handler.callback.onEnvelope(obj.get("type").getAsString(), obj.get("data"));
                return;
            }

            // Output line from the command (legacy tag-line protocol, kept as a
    ...
    private void handleDaemonEvent(JsonObject obj) {
        ...
        switch (event) {
            case "ready":
                ...
                for (DaemonLifecycleListener listener : lifecycleListeners) {
                    try {
                        listener.onDaemonReady();
                    } catch (Exception ex) {
                        LOG.warn("[DaemonBridge] Lifecycle listener threw on ready", ex);
                    }
                }
                break;
    ...
    private void notifyEventListeners(String event, JsonObject obj) {
    ...
    private void handleDaemonDeath() {
        ...
        for (DaemonLifecycleListener listener : lifecycleListeners) {
            try {
                listener.onDaemonDied();
            } catch (Exception ex) {
                LOG.warn("[DaemonBridge] Lifecycle listener threw on died", ex);
            }
        }
    ...
    /**
     * Register an additional lifecycle listener. Multiple listeners may
     * coexist; each is invoked after the primary {@link #setLifecycleListener}
     * listener. Callers MUST pair this with {@link #removeLifecycleListener}
     * on disposal to avoid memory leaks.
     */
    public void addLifecycleListener(DaemonLifecycleListener listener) {
        if (listener == null) { return; }
        lifecycleListeners.add(listener);
    }

    /**
     * Remove a previously registered lifecycle listener. No-op if not registered.
     */
    public void removeLifecycleListener(DaemonLifecycleListener listener) {
        if (listener == null) { return; }
    ...
    /**
     * Test hook: register a pending request handler directly, without a running
     * daemon, so handleDaemonOutput can be exercised in unit tests.
     */
    void addPendingRequestForTest(String requestId, DaemonOutputCallback callback) {
        pendingRequests.put(requestId, new RequestHandler(callback, new CompletableFuture<>()));
    }
    ...
    public interface DaemonOutputCallback {
        ...
        /**
         * Called when a structured v2 envelope ({id, type, data}) is received
         * for this request. The default implementation ignores envelopes so
         * callbacks written against the legacy tag-line protocol keep working.
         *
         * @param type Envelope type (e.g., "message", "content_delta", "send_error")
         * @param data Envelope payload; null for marker types without data
         */
        default void onEnvelope(String type, JsonElement data) {
        }
