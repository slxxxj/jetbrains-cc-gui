...
public abstract class BaseSDKBridge {
    ...
    /**
     * Process a structured v2 envelope ({type, data}) emitted by the Node bridge.
     * Subclasses mirror their {@link #processOutputLine} branch semantics here.
     *
     * @param type             Envelope type (e.g., "message", "content_delta")
     * @param data             Envelope payload; null for marker types
     * @param callback         Message callback
     * @param result           SDK result being built
     * @param assistantContent StringBuilder for accumulating assistant content
     * @param hadSendError     Flag indicating if send error occurred
     * @param lastNodeError    Holder for the last Node.js error
     */
    protected abstract void processEnvelope(
            String type,
            JsonElement data,
            MessageCallback callback,
            SDKResult result,
    ...
    /**
     * Try to dispatch a stdout line as a structured v2 envelope ({type, data}).
     *
     * @return true when the line was an envelope and has been dispatched;
     *         false when it is a legacy tag line or non-JSON output and the
     *         caller should fall back to {@link #processOutputLine}
     */
    protected boolean tryProcessEnvelope(
            String line,
            MessageCallback callback,
            SDKResult result,
            StringBuilder assistantContent,
