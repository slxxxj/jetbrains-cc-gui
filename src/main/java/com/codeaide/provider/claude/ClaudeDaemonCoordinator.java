...
class ClaudeDaemonCoordinator {
    ...
    private final List<DaemonBridge.DaemonLifecycleListener> cachedLifecycleListeners = new CopyOnWriteArrayList<>();
    ...
    /**
     * Register a daemon lifecycle listener (ready/died). Cached so it is
     * re-attached to any future daemon bridge created after a restart.
     */
    void addDaemonLifecycleListener(DaemonBridge.DaemonLifecycleListener listener) {
        if (listener == null) { return; }
        cachedLifecycleListeners.add(listener);
        DaemonBridge current = daemonBridge;
        if (current != null && current.isAlive()) {
    ...
    /**
     * Unregister a previously added lifecycle listener. No-op if not registered.
     */
    void removeDaemonLifecycleListener(DaemonBridge.DaemonLifecycleListener listener) {
        if (listener == null) { return; }
        cachedLifecycleListeners.remove(listener);
        DaemonBridge current = daemonBridge;
        if (current != null && current.isAlive()) {
        ...
        synchronized (daemonLock) {
            ...
            try {
                ...
                if (newBridge.start()) {
                    ...
                    for (DaemonBridge.DaemonLifecycleListener cached : cachedLifecycleListeners) {
                        newBridge.addLifecycleListener(cached);
                    }
