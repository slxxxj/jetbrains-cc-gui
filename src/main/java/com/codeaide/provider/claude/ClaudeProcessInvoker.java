...
class ClaudeProcessInvoker {
    ...
    private void readOutput(
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            // Per-process mode has no daemon abort channel; send_error suppression
            // via wasAborted only applies in daemon mode, hence always false here.
            AtomicBoolean wasAborted = new AtomicBoolean(false);
            String line;
            while ((line = reader.readLine()) != null) {
                // Structured v2 envelopes ({type, data}) take precedence over
                // legacy tag lines.
                JsonObject envelope = tryParseEnvelopeLine(line);
                if (envelope != null) {
                    String type = envelope.get("type").getAsString();
                    if ("send_error".equals(type)) {
                        String errorMessage = appendEnvironmentDiagnostics(
                                extractEnvelopeError(envelope.get("data")), node, nodeVersion, workDir);
    ...
    /**
     * Parse a stdout line as a structured v2 envelope ({type, data}).
     * Returns the envelope object when the line is JSON carrying a string
     * "type" field, or null for legacy tag lines / plain text / other JSON.
     */
    private JsonObject tryParseEnvelopeLine(String line) {
    ...
    private String extractEnvelopeError(JsonElement data) {
        ...
        return data != null && !data.isJsonNull() ? data.toString() : "Unknown error";
    }
    ...
    private String formatSendError(String line, String node, String nodeVersion, File workDir) {
        ...
        return appendEnvironmentDiagnostics(errorMessage, node, nodeVersion, workDir);
    }

    private String appendEnvironmentDiagnostics(String errorMessage, String node, String nodeVersion, File workDir) {
