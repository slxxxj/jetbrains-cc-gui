package com.codeaide.notifications;
...
public class ClaudeStatusBarWidget implements CustomStatusBarWidget, StatusBarWidget, Disposable {
        ...
        if (label == null) {
            ...
            label.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    ...
                    var toolWindow = ToolWindowManager.getInstance(project).getToolWindow("CodeAide");
                    if (toolWindow != null) {
    ...
    private void refreshDisplay(String details) {
        ...
        if ("thinking".equals(status)) {
            statusText = CodeAideBundle.message("status.thinking");
        } else if ("generating".equals(status)) {
            statusText = CodeAideBundle.message("status.generating");
        } else if ("waiting".equals(status)) {
            statusText = CodeAideBundle.message("status.waiting");
        ...
        if (mode != null && !"default".equals(mode)) {
            String modeLabel = switch (mode) {
                case "plan" -> CodeAideBundle.message("status.mode.plan");
                case "acceptEdits" -> CodeAideBundle.message("status.mode.acceptEdits");
                case "bypassPermissions" -> CodeAideBundle.message("status.mode.bypassPermissions");
                default -> mode;
            };
        ...
        StringBuilder tooltip = new StringBuilder(CodeAideBundle.message("status.tooltip.status", status));
        if (model != null && !model.isEmpty()) {
            tooltip.append(CodeAideBundle.message("status.tooltip.model", model));
        }
        if (mode != null) {
            tooltip.append(CodeAideBundle.message("status.tooltip.mode", mode));
        }
        if (agent != null && !agent.isEmpty()) {
            tooltip.append(CodeAideBundle.message("status.tooltip.agent", agent));
        }
        if (details != null) {
            tooltip.append(CodeAideBundle.message("status.tooltip.details", details));
        }
    ...
    public static class Factory implements StatusBarWidgetFactory {
        ...
        public @NotNull String getDisplayName() {
            return CodeAideBundle.message("status.widgetName");
        }
        ...
        public boolean isAvailable(@NotNull Project project) {
            ...
            try {
                return new CodeaideSettingsService().getStatusBarWidgetEnabled();
            } catch (Exception e) {
                return true;
            }
        }
