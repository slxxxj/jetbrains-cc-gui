...
import com.codeaide.util.TokenUsageUtils;
...
public class ClaudeMessageHandler implements MessageCallback {
    ...
    private void handleAssistantMessage(String content) {
        ...
        try {
            ...
            // During streaming the single-message upsert fires FIRST: it activates
            // the coalescer's upsert mode, which turns the subsequent full-list
            // notifyMessageUpdate into a flush-only snapshot (no mid-stream full push).
            callbackHandler.notifyMessageUpsert(currentAssistantMessage);
            callbackHandler.notifyMessageUpdate(state.getMessages());
    ...
    private void handleUserMessage(String content) {
        ...
        try {
            ...
            if (messageParser.hasToolResult(userMsg)) {
                ...
                callbackHandler.notifyMessageUpsert(toolResultMessage);
                callbackHandler.notifyMessageUpdate(state.getMessages());
                return;
            }
    ...
    /**
     * Handle a tool_preparing signal: the model has started a tool_use block and
     * is now streaming its (potentially large) input JSON. Forwarded to the
     * frontend so it can show a "preparing tool call" hint during the silent
     * argument-generation phase. Payload: {"name": "...", "index": N}.
     */
    private void handleToolPreparing(String content) {
        String toolName = "";
        try {
            JsonObject obj = gson.fromJson(content, JsonObject.class);
            if (obj != null && obj.has("name") && !obj.get("name").isJsonNull()) {
                toolName = obj.get("name").getAsString();
            }
        } catch (Exception e) {
            LOG.warn("Failed to parse tool_preparing JSON: " + e.getMessage());
        }
        callbackHandler.notifyToolPreparing(toolName);
    }

    /**
     * Handle a compact_status signal: context compaction started
     * (compacting=true) or ended (compacting=false). Forwarded to the frontend
     * so it can indicate "compacting context" instead of looking frozen.
     * Payload: {"compacting": true|false, "trigger"?: "manual"|"auto"}.
     */
    private void handleCompactStatus(String content) {
        boolean compacting = false;
        try {
            JsonObject obj = gson.fromJson(content, JsonObject.class);
            if (obj != null && obj.has("compacting") && !obj.get("compacting").isJsonNull()) {
                compacting = obj.get("compacting").getAsBoolean();
            }
        } catch (Exception e) {
            LOG.warn("Failed to parse compact_status JSON: " + e.getMessage());
        }
        callbackHandler.notifyCompactStatus(compacting);
    }
    ...
    private void handleThinkingDelta(String content) {
        ...
        if (applied) {
            ...
            // The upsert carries just this assistant message (incremental);
            // the full list stays as the coalescer's end-of-stream flush candidate.
            callbackHandler.notifyMessageUpsert(currentAssistantMessage);
            callbackHandler.notifyMessageUpdate(state.getMessages());
        } else {
