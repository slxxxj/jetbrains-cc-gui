...
public class SessionCallbackAdapter implements ClaudeSession.SessionCallback {
    ...
    @Override
    public void onMessageUpsert(ClaudeSession.Message message) {
        if (isInactive() || message == null) {
            return;
        }
        streamCoalescer.enqueueUpsert(message);
    }
    ...
    @Override
    public void onToolPreparing(String toolName) {
        if (isInactive()) {
            return;
        }
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            jsTarget.callJavaScript("onToolPreparing", JsUtils.escapeJs(toolName != null ? toolName : ""));
            LOG.debug("Tool preparing sent to frontend: " + toolName);
        });
    }

    @Override
    public void onCompactStatus(boolean compacting) {
        if (isInactive()) {
            return;
        }
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            jsTarget.callJavaScript("onCompactStatus", String.valueOf(compacting));
            LOG.debug("Compact status sent to frontend: " + compacting);
        });
    }
