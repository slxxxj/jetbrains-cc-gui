...
public class CodexMessageHandler implements MessageCallback {
    ...
    private void handleAssistantMessage(String jsonContent) {
        try {
            ...
            if (currentAssistantMessage != null) {
                ...
                affected = currentAssistantMessage;
            } else {
                state.addMessage(parsed);
                affected = parsed;
            }
            // Structural change (tool_use / thinking / text snapshot): push the
            // single affected message via the incremental upsert channel FIRST —
            // the first upsert of a stream activates the coalescer's upsert mode,
            // which turns the full-list update below into a flush-only snapshot
            // (no mid-stream O(conversation) re-push).  Both branches are plain
            // in-place mutation / append, so single-message upsert semantics hold.
            callbackHandler.notifyMessageUpsert(affected);
            callbackHandler.notifyMessageUpdate(state.getMessages());
    ...
    private void handleUserMessage(String jsonContent) {
        try {
            ...
            state.addMessage(parsed);
            // Appended tool_result user message: incremental upsert first (the
            // frontend dedups these by tool_use_id), then the full-list update
            // which upsert mode reduces to a flush-only snapshot mid-stream.
            callbackHandler.notifyMessageUpsert(parsed);
            callbackHandler.notifyMessageUpdate(state.getMessages());
    ...
    private void handleContentDelta(String content) {
        ...
        // (onContentDelta) provides real-time character-by-character display,
        // and per-delta full-list pushes are exactly the O(conversation) IPC
        // cost the upsert design removes (mirrors ClaudeMessageHandler).
        //
        // After streaming ends (isStreaming=false), we MUST still notify:
        // deltas can arrive after handleStreamEnd fired — without this call
    ...
    private void handleThinkingDelta(String content) {
        ...
        // Thinking blocks are structural: the frontend renders them from raw
        // blocks in collapsible UI sections, so the mutated assistant message
        // travels as an incremental upsert (in-place mutation semantics).
