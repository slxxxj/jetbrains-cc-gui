...
import com.codeaide.skill.SlashCommandRegistry;
...
public class SessionLifecycleManager {
    ...
    private SessionProviderRouter providerRouter;
    ...
    /**
     * Provider registry built over the host's bridges. Lazily created because
     * the host's bridges may not be wired at construction time.
     */
    private SessionProviderRouter providerRouter() {
        if (providerRouter == null) {
            providerRouter = new SessionProviderRouter(host.getClaudeSDKBridge(), host.getCodexSDKBridge());
        }
        return providerRouter;
    }

    /**
     * Reset persistent runtimes on every daemon-capable provider (today:
     * claude only). Runs unconditionally on session teardown regardless of the
     * session's own provider — a tab may hold a warm Claude runtime even while
     * its current session is Codex.
     */
    private void resetDaemonPersistentRuntime(String runtimeSessionEpoch) {
        for (ProviderOps ops : providerRouter().all()) {
            if (ops.capabilities().supportsDaemon()) {
                ops.resetPersistentRuntime(runtimeSessionEpoch);
            }
        }
    }

    /**
     * Prewarm the daemon runtime on every daemon-capable provider (today:
     * claude only).
     */
    private void prewarmDaemonAsync(String cwd, String runtimeSessionEpoch, String sessionId) {
        for (ProviderOps ops : providerRouter().all()) {
            if (ops.capabilities().supportsDaemon()) {
                ops.prewarmDaemon(cwd, runtimeSessionEpoch, sessionId);
            }
        }
    }
    ...
    public void createNewSession() {
        ...
        interruptFuture.thenRun(() -> {
            if (oldSession != null) {
                resetDaemonPersistentRuntime(oldSession.getRuntimeSessionEpoch());
    ...
    public void loadHistorySession(String sessionId, String projectPath, String provider) {
        ...
        interruptFuture.thenRun(() -> {
            ...
            prewarmDaemonAsync(workingDir, newSession.getRuntimeSessionEpoch(), sessionId);
    ...
    private void resetTokenUsage() {
        ...
        if (browser != null && !host.isDisposed()) {
            String js = JsUtils.buildJsCall("window.onUsageUpdate", JsUtils.escapeJs(usageJson));
            browser.getCefBrowser().executeJavaScript(js, browser.getCefBrowser().getURL(), 0);
        }
    }

    private String getCurrentEditorFilePath() {
        return com.codeaide.util.EditorFileUtils.getCurrentEditorFilePath(this.host.getProject());
    }
    ...
    private void completeNewSessionBootstrap(ClaudeSession newSession, String workingDirectory, String successLogPrefix) {
        ...
        prewarmDaemonAsync(workingDirectory, newSession.getRuntimeSessionEpoch(), null);
        ...
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            host.callJavaScript("historyLoadComplete");
            host.callJavaScript("updateStatus",
                    JsUtils.escapeJs(CodeAideBundle.message("toast.newSessionCreatedReady")));
            resetTokenUsage();
        });
    }
}
